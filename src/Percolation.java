...
public class Percolation {
    ...
    private boolean mPercolated = false;
    ...
    public Percolation(int n) {
        ...
        mUf = new WeightedQuickUnionUF(n*n+3);

        mSize = n;
        mBoolGrid = new boolean[n*n+3];
        mBoolGrid[0] = true;
    ...
    public void open(int row, int col) {
        ...
        if (!mBoolGrid[elementIndex]) {
            ...
            // connects to the bottom virtual element if it is full (percolates to the top)
            // and it is the first element to do it.
            if (row == mSize) {
    ...
    public boolean percolates() {
        // when it is not percolating, always test all opened bottom sites
        // when it begins percolating, just retrieve a variable
        if (!mPercolated) {
            mPercolated = mUf.connected(0, mSize*mSize+1);
            // the first time it percolates
            // check if any opened bottom site is full
            if (mPercolated) {
                // connects the first full site with the new bottom site
                // and uses it from now on
                for (int i = 1; i <= mSize; i++) {
                    if (isOpen(mSize, i) && isFull(mSize, i)) {
                        mUf.union(ijto1d(mSize, i), mSize*mSize+2);
                        break;
                    }
                }
            }
            ...
            return mUf.connected(0, mSize*mSize+2);
    }
